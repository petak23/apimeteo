...
final class ChartPresenter extends BasePresenter
{
	...
	private function drawCoverageSeries($col, $row, $serie, $year, $sensor, $colors)
	{
		...
		$text = "{$year}";
		$this->image->ttfText(15, 0, $x, $y,  ImageColor::rgb(0, 0, 0), $this->fontNameBold, $text);

		$x = $x + 60;
		$y = $y - 1;
		$text = substr($sensor->desc, 0, 31);
		$this->image->ttfText(10, 0, $x, $y,  ImageColor::rgb(0, 0, 0), $this->fontName, $text);
	...
	public function renderSensor(
		...
		if ($params->altSeries()) {
			$vi2 = [
				...
				'unit' => $sensor['unit'],
				'source_desc' => $viewSource['short_desc'],
				'color' => (new Model\Color(0, 0, 255))->getHtmlColor(),
				'date' => $params->altDateFrom->format('d.m.Y'),
				'nr' => 1
			];
			$outView[] = $vi2;
		}

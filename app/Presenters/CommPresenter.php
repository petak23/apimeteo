	...
	public function actionDatajson(): void
	{
		...
		try {
			...
			if( $control_hash !== $json_msg["payload_hash"]  ) {
				$logger->write( Logger::ERROR,  "CommPresenter:actionDatajson:Ex=> ERR: Not valid sha256 of message! Expected: {$control_hash}, Given: {$json_msg['payload_hash']}" );
				$logger->write( Logger::ERROR,  "CommPresenter:actionDatajson:Ex=> ERR: Message: {$str_message}" );
				throw new \Exception("Not valid sha256 of message! [" . $str_message . "] Expected: {$control_hash}, Given: {$json_msg['payload_hash']}");
			}
